        ...
        <section className="mb-8">
            <div className="glass-card p-6 sm:p-8">
                ...
                {loading && comments.length === 0 ? (
                    ...
                    <div className="text-center py-12 border-t border-border-subtle mt-6">
                        <p className="text-text-muted">No comments yet. Be the first to join the conversation!</p>
                    </div>
                ) : (
                ...
                <div className="flex-shrink-0">
                    {comment.user.profileImage ? (
                        <img
                            ...
                            className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border border-border-subtle"
                        />
                    ) : (
                    ...
                    {replies.length > 0 && (
                        <div className="w-0.5 h-full bg-border-subtle mx-auto mt-2 group-last:hidden" />
                    )}
                </div>

                <div className="flex-1 min-w-0">
                    <div className="bg-bg-secondary/30 rounded-lg p-3 sm:p-4 border border-border-subtle/50 hover:border-border-subtle transition-colors">
                        {/* Header */}
                        ...
                        {/* Text */}
                        <p className="text-text-secondary text-sm sm:text-base leading-relaxed whitespace-pre-wrap break-words">
                            {comment.text}
                        </p>
                        ...
                        <div className="flex items-center gap-4 mt-3">
                            <button
                                ...
                                className="flex items-center gap-1.5 text-xs sm:text-sm text-text-muted hover:text-primary transition-colors"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button
                                ...
                                className="text-xs sm:text-sm text-text-muted hover:text-text-primary transition-colors font-medium"
                            >
                    ...
                    {/* Reply Form */}
                    {isReplying && (
                        <div className="mt-3 pl-2 border-l-2 border-border-subtle">
                            <CommentForm
...
function CommentForm({ onSubmit, autoFocus, placeholder, onCancel }: { onSubmit: (text: string) => Promise<void> | void, autoFocus?: boolean, placeholder?: string, onCancel?: () => void }) {
    ...
    if (!user) {
        return (
            <div className="bg-bg-secondary/30 p-4 rounded-lg text-center border border-border-subtle">
    ...
    return (
        <form onSubmit={handleSubmit} className="relative">
            <textarea
                ...
                rows={autoFocus ? 2 : 3}
                autoFocus={autoFocus}
                className="w-full px-4 py-3 bg-bg-secondary border border-border-card rounded-lg text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none transition-all"
            />
