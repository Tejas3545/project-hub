...
export default async function HomePage() {
  ...
  try {
    ...
    domains = await domainApi.getAll();
    console.log('Fetched domains count:', domains.length);
  } catch (error) {
  ...
  return (
    <div className="relative min-h-screen bg-[#030303] overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16 relative z-10">
        ...
        {/* Bottom CTA */}
        <div className="pro-card p-8 sm:p-12 lg:p-16 text-center relative overflow-hidden group">
          <div className="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          <div className="relative z-10">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold mb-4 sm:mb-6 text-white px-4">
              Ready to Start Building?
            </h2>
            <p className="text-sm sm:text-base lg:text-lg text-text-secondary mb-6 sm:mb-8 max-w-2xl mx-auto leading-relaxed px-4">
              Each project comes with complete specifications, industry context, skill requirements,
              and evaluation criteria to guide your learning journey.
            </p>
            <Link href="/projects">
              <Button className="h-12 px-8 w-full sm:w-auto">Start Your Journey</Button>
            </Link>
          </div>
        </div>
      </div>
